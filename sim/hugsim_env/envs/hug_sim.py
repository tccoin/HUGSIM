    ...
    def __init__(self, cfg, output):
        ...
        if self.sg_enabled and self.sg_backend is None:
            ...
            if backend_name == 'alpasim':
                ...
                backend_kwargs = OmegaConf.to_container(sg_cfg, resolve=True)
                for non_backend_key in ('enabled', 'backend', 'use_exact_ego_pose'):
                    backend_kwargs.pop(non_backend_key, None)
                self.sg_backend = InstantGSWorldRenderBackend(**backend_kwargs)
