...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        if cfg.MODEL.DOWN_CONV:
            self.conv0 = nn.Conv2d(3, 3, kernel_size=2, stride=2, padding=0, bias=False)
        ...
        self.high_level_ch = np.int_(np.sum(pre_stage_channels))
        self.stage2_channels = np.int_(np.sum(pre_stage_channels[:2]))
        self.stage3_channels = np.int_(np.sum(pre_stage_channels[:3]))
    ...
    def forward(self, x):
        ...
        ys2_1 = F.interpolate(y_list[1], size=(x0_h, x0_w),
                           mode='bilinear', align_corners=align_corners)

        feats_stage2 = torch.cat([y_list[0], ys2_1], 1)        
        ...
        # Upsampling
        x0_h, x0_w = y_list[0].size(2), y_list[0].size(3)
        ys3_1 = F.interpolate(y_list[1], size=(x0_h, x0_w),
                           mode='bilinear', align_corners=align_corners)
        ys3_2 = F.interpolate(y_list[2], size=(x0_h, x0_w),
                           mode='bilinear', align_corners=align_corners)

        feats_stage3 = torch.cat([y_list[0], ys3_1, ys3_2], 1)
        ...
        return feats_stage2, feats_stage3, feats
